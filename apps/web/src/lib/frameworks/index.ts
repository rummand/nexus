 ...
 * problem, how the organisation is run, and then the estate itself. Nothing here is required — a
...
export function byFamily(): Array<{ family: FrameworkFamily; frameworks: Framework[] }> {
  const order: FrameworkFamily[] = ["notation", "domain", "operating-model", "portfolio"];
  return order
