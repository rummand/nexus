  ...
  blocks: [
    { kind: "prose", text: "Architecture vocabulary is overloaded — every word means three things depending on who is in the room. These are the meanings Nexus uses." },
    {
      ...
      rows: [
        ...
        ["Session", "Being signed in on one browser. Ending it signs out that browser only; changing a password ends every one."],
...
export const FAQ: DocPage = {
  ...
  keywords: ["faq", "troubleshooting", "help", "problem", "conflict", "backup", "collaboration", "undo", "import", "intake", "model", "api key", "sign in", "password", "account", "log in"],
  blocks: [
    { kind: "heading", text: "Two of us opened the same board. What happens?", id: "conflict" },
    { kind: "prose", text: "You work on it together — you see each other's cursors, and what each of you changes appears on the other's screen. The rules are on *Two people on one board*. The one thing that does not merge is text: a field somebody is typing in is locked to them, because merging characters loses them." },
    { kind: "prose", text: "If the shared connection cannot be made — an old proxy, a network that will not carry it — Nexus falls back to the way it used to work: your tab saves for itself, and whoever saves second is refused with “Changed elsewhere — reload” rather than overwriting the first. Refusing is the only honest answer when the two tabs genuinely cannot see each other." },
    ...
    { kind: "heading", text: "How do I sign in, and what does Nexus know about me?", id: "signin" },
    { kind: "prose", text: "Your email address and a password, which is stored as a **scrypt** hash and never in a form anybody can read back — not an administrator, not the database. Signing in starts a session that lasts thirty days and renews while you are using it; **Sign out** in the sidebar or the board topbar ends it, and ends it for anything holding a copy of the cookie rather than merely forgetting it here." },
    { kind: "note", tone: "why", title: "Why the sign-in page will not tell you whether an account exists", text: "“No account with that address” is a way to find out who works somewhere, and this product knows the answer for a whole organisation. So a wrong email and a wrong password give the same sentence, and take the same length of time — the second part matters, because otherwise the clock answers the question the wording refuses to." },
