...
import { Magnet, Maximize2, Minus, Mouse, Plus, Touchpad } from "lucide-react";
...
export function ZoomCard() {
  ...
  const snap = useCanvas((s) => s.snapEnabled);
  ...
  return (
    <section className="floating-panel zoom-card" aria-label="Zoom controls" onPointerDown={(e) => e.stopPropagation()} onDoubleClick={(e) => e.stopPropagation()}>
      ...
      <button type="button" className={snap ? "active" : ""} title={snap ? "Smart guides on (hold Alt to bypass). Click to turn off." : "Smart guides off. Click to turn on."} onClick={() => store.getState().setSnapEnabled(!snap)}><Magnet size={18} /></button>
