...
export default async function GraphPage({ params, searchParams }: { params: Promise<{ slug: string }>; searchParams: Promise<{ entity?: string }> }) {
  ...
  const [snapshot, { spaces }, proposals, entities, relations, run, choice, providers] = await Promise.all([
    ...
    choose(db, workspace.id, "graph agent"),
  ...
  const agent = {
    ...
    hint: choice ? "" : whyNoModel(await configured(db, workspace.id), providers, "graph agent"),
    lastAskedAt: run?.at ?? null,
